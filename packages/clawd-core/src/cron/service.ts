...
function normalizeRequiredName(raw: unknown): string {
...
function normalizePayloadToSystemText(payload: CronPayload): string {
...
export class CronService {
  ...
  async update(id: string, patch: CronJobPatch): Promise<CronJob> {
    return await this.locked(async () => {
      ...
      if (patch.isolation)
        job.isolation = { ...job.isolation, ...patch.isolation };
  ...
  async run(
    id: string,
    mode?: "due" | "force",
  ): Promise<{ ok: boolean; ran: boolean; reason?: "not-due" }> {
    return await this.locked(async () => {
  ...
  private warnIfDisabled(action: string): void {
  ...
  private findJobOrThrow(id: string): CronJob {
